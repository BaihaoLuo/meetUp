...
public class Schedule {
    ...
    public Boolean haveTimeToMeetUp(String dayOfWeek, String timeOfDay) {
        String meetUpTime = timeOfDay + ":00";
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        if(sections.isEmpty()){
            return false;
        }
        SortedSet<String> sectionStartTimes = new TreeSet<String>();
        for (Section s : sections) {
            String startTime = s.getCourseTime().getStartTime();
            sectionStartTimes.add(startTime);
        }
        if (sectionStartTimes.contains(meetUpTime)) {
            return false;
        }
        String startTimeString = startTime(dayOfWeek).getStartTime();
        if(calculateBreakTimeInMinutes(startTimeString,meetUpTime) >= 0){
            return false;
        }
        for (String str : sectionStartTimes) {
            if (calculateBreakTimeInMinutes(str, meetUpTime) >= 60) {

                return true;
            }
        }
        Section latestSection = sections.last();
        String endTimeOfLastSection = latestSection.getCourseTime().getEndTime();

        if(calculateBreakTimeInMinutes(meetUpTime,endTimeOfLastSection) >= 0){
            return true;
        }
